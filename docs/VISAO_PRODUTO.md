...
| **Financeiro / gestão** | Títulos a receber, PIX/boleto (Asaas), inadimplência, fila a faturar, exportação contábil (CSV, **Omie**, **Bling**, Sisloc legado), ponte fiscal |
| **Logística** | Lista do dia, mapa de obras, romaneio, frota de entrega, pátios |
...
- **Pipeline** de oportunidades (lead → proposta → locação)
- **Campanha de clientes inativos** com mensagens enfileiradas
- **Follow-up** e lembretes agendados
- WhatsApp/SMS: driver `log` (dev), `link` (wa.me) ou **Twilio** (produção)

### Copiloto operacional

Assistente interno (painel flutuante + **Configurações → Copiloto**) para quem tem permissão `agent.api`:
...
|-------------|----------------|
| Emitir **NF-e / NFS-e** no próprio sistema | Não — exportação + ponte para **Omie/Bling**; NF emitida no ERP fiscal |
| Substituir **Protheus** (RH, compras, contabilidade completa) | Não — escopo é operação + comercial + financeiro leve |
| **Rotas otimizadas** multi-cidade | Parcial — romaneio e mapa de obras existem; otimização de rota no roadmap |
| **Boleto/PIX automático** | Implementado via **Asaas** — requer configuração em produção ([GO_LIVE.md](GO_LIVE.md)) |
| **App nativo** iOS/Android | Modo PWA no navegador; app dedicado no roadmap |
| **CRM completo** | Parcial — pipeline, campanhas e mensagens; sem Serasa nem múltiplos contatos por cliente |
| **Permissão por filial/pátio** | Não — multi-empresa (CNPJ) sim; escopo por pátio no roadmap |
...
1. **Go-live** — deploy em produção, Asaas, transição fiscal validada ([GO_LIVE.md](GO_LIVE.md)).
2. **CRM** — Twilio em produção, copiloto com comandos CRM, múltiplos contatos por cliente.
3. **Plataforma** — API REST pública, webhooks de eventos, permissão por pátio/filial.
4. **Fiscal** — webhook de NF emitida vinculando número da nota ao título.
5. **Logística** — otimização de rotas e agenda de entrega avançada.
...
O menu superior está organizado em **seis seções**: Dashboard, Comercial, Logística, Estoque, Financeiro e Configurações. Cada seção agrupa as telas do dia a dia da locadora.

---

*Última revisão: junho/2026 — alinhada às fases 1–15 e go-live.*
